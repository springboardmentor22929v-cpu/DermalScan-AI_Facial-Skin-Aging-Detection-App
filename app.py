# File: app.py (Main Streamlit Application - Combines Frontend and Backend Inference) 
...
# Correct model path for your trained DermalScan model 
...
def load_dermalscan_model(): 
    try: 
        # Suppress TensorFlow messages during loading 
        tf.get_logger().setLevel('ERROR')  
        model = tf.keras.models.load_model(DERMALSCAN_MODEL_PATH) 
        tf.get_logger().setLevel('INFO') # Reset logger level 
...
def predict_aging_signs_and_annotate(input_image_pil): 
    ...
    if dermalscan_model is None or mtcnn_detector is None: 
        st.error("AI Models failed to load. Please check logs/paths.") 
    ...
    if not faces: 
        ...
        return draw_img_pil, [] # Return original image with message, empty predictions 
    ...
    for i, face in enumerate(faces): 
        ...
        # Sort by confidence for primary label for image annotation 
        ...
        try: 
            ...
            font_path = os.path.join(os.path.dirname(__file__), "arial.ttf") # Assumes arial.ttf is in same dir as app.py 
            if not os.path.exists(font_path): 
                font = ImageFont.load_default() # Fallback 
            else: 
                font = ImageFont.truetype(font_path, size=max(12, int(input_image_pil.width / 40))) 
        except IOError: 
...
if uploaded_file is not None: 
    ...
    st.image(image, caption="Uploaded Image for Analysis", use_column_width=True) 
        ...
        with st.spinner("Analyzing image... This may take a few seconds as the AI processes your facial features."): 
            ...
            if predictions: 
                if predictions[0].get("message") == "No face detected in the image.": 
                    ...
                    st.subheader("📸 Annotated Image with Predictions:") 
